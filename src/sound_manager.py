...
class SoundManager:
    ...
    def __init__(self):
        ...
        self._sound_types = {
            ...
            "PAC-MAN": SoundType.MUSIC,
            "credit": SoundType.MUSIC,
            
            # Interface
            "miss": SoundType.UI,
            "coffee-break-music": SoundType.UI,
    ...
    def play_sound(self, sound_name: str, volume: Optional[float] = None, fade_ms: int = 1000) -> bool:
        """
        Reproduz um som específico. Para músicas de fundo, faz loop eterno com fade.
        Args:
            sound_name: Nome do som a ser reproduzido
            volume: Volume específico (opcional, usa volume padrão se não fornecido)
            fade_ms: Duração do fade-in em milissegundos (padrão 1000ms)
        Returns:
        ...
        try:
            ...
            # Se for música, faz loop infinito com fade
            sound_type = self._sound_types.get(sound_name, SoundType.EFFECT)
            if sound_type == SoundType.MUSIC:

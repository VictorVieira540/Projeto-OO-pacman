...
class MovableObject(GameObject):
    ...
    def can_move(self, direction, game_map, entity_type=None):
        ...
        next_pos = Vector2D(
            self._position.x + direction.value[0] * 2,
            self._position.y + direction.value[1] * 2
        )
...
class Ghost(MovableObject):
    def __init__(self, x, y, color, size, speed, initial_position, ghost_type="red", patrol_route=None):
        super().__init__(x, y, color, size, speed)
        self._state = "normal"
        ...
        #chase: persegue Pac-Man
        #scatter: persegue Pac-Man
        #frightened: fugi de Pac-Man
        #eaten: aguarda 5 segundos no spawn
        ...
        if patrol_route is not None:
            self._patrol_route = patrol_route
        else:
    ...
    def set_difficulty(self, difficulty_level):
        ...
        difficulty_factor = min(difficulty_level / 200.0, 1.0)
        self._difficulty_multiplier = 1.0 + difficulty_factor * 2.0  # multiplicador muito agressivo
        timing_factor = 1.0 - (difficulty_factor * 0.5)
        self._astar_frequency = max(int(80 * timing_factor), 20)  # recalculo quase instantâneo
        
        # Aplicar multiplicador de velocidade baseado na dificuldade
        # Velocidade máxima limitada para não ultrapassar a velocidade do player (1.3)
        max_speed = 1.3  # velocidade do player
        speed_multiplier = 1.0 + (difficulty_factor * 0.08)  # até 8% mais rápido
        target_speed = self._base_speed * speed_multiplier
        self._speed = min(target_speed, max_speed)

    def update_difficulty(self, pellets_eaten_percentage):
        """Método legado - mantido para compatibilidade"""
        pass

    def get_difficulty_adjusted_vulnerable_duration(self, base_duration=10000):
        reduction_factor = 1.0 - (self._difficulty_multiplier - 1.0) * 2.0  # reduz muito rápido
        adjusted_duration = int(base_duration * reduction_factor)
        return max(adjusted_duration, 500)

    def get_difficulty_adjusted_mode_timing(self):
        """Retorna timings de modo ajustados pela dificuldade"""
        base_patrol = 1000   # 8 segundos patrulhando (reduzido)
        base_chase = 25000   # 25 segundos perseguindo (aumentado)
        difficulty_factor = self._difficulty_multiplier - 1.0
        patrol_duration = int(base_patrol * (1.0 - difficulty_factor * 0.6))  # reduz mais com dificuldade
        chase_duration = int(base_chase * (1.0 + difficulty_factor * 1.5))    # aumenta muito mais com dificuldade
        patrol_duration = max(patrol_duration, 2000)  # mínimo 2 segundos
        chase_duration = min(chase_duration, 60000)   # máximo 60 segundos
    ...
    def advance_patrol_waypoint(self):
        ...
        #tolerância para o waypoint
        tolerance = {
            "red": 40,
            "pink": 50,
            "cyan": 45,
            "orange": 60
        }.get(self._ghost_type, 48)
    ...
    def reset_position(self):
        ...
        self._state = "normal"
        # Manter velocidade ajustada pela dificuldade
        difficulty_factor = (self._difficulty_multiplier - 1.0) / 2.0  # converter de volta para fator 0-1
        max_speed = 1.3  # velocidade do player
        speed_multiplier = 1.0 + (difficulty_factor * 0.08)  # até 8% mais rápido
        target_speed = self._base_speed * speed_multiplier
        self._speed = min(target_speed, max_speed)

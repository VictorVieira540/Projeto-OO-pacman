...
def stop_all_except_music():
    for sound_name in sound_manager._sounds:
        sound_type = sound_manager._sound_types.get(sound_name)
        if sound_type in [SoundType.EFFECT, SoundType.UI, SoundType.GHOST]:
            sound_manager.stop_sound(sound_name)
...
class Game:
    ...
    def _initialize_game(self):
        ...
        for config in ghost_configs:
            ...
            patrol_route = self._map.get_random_waypoints(n=8)
            ghost = Ghost(
                x=ghost_pos.x,
                y=ghost_pos.y,
                color=config["color"],
            ...
            if event.type == pygame.QUIT:
                ...
                if self._state == GameState.MENU:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        self._previous_state = GameState.PAUSED
                        self._state = GameState.OPTIONS
                    return True
    ...
    def update(self, delta_time):
        ...
        if self._state == GameState.PLAYING:
            ...
            # DEBUG: Avança de nível ao comer apenas um pellet
            #if len(pellets_to_remove) > 0:
            #   self._pellets.clear()
            ...
            if len(self._pellets) == 0:
                ...
                if self._current_map_index + 1 < len(self._available_maps):
                    ...
                    sound_manager.play_sound("coffee-break-music")
    ...
    def render(self):
        ...
        if self._state == GameState.MENU:
            ...
            self._draw_text_centered("O - Opções", self._font_small, (255, 255, 255), 50)
